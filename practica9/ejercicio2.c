...
int main( int argc, char *argv[] ) {
  ...
  if (miId == 0) { //proceso distribuidor
    ...
    for(i=0; i < dimVectorNumeros; i++){
      MPI_Recv(&token, 1, MPI_CHAR, MPI_ANY_SOURCE, 88, MPI_COMM_WORLD, &status);
      //printf("esperando que el proce 0 reciba de %d\n", status.MPI_SOURCE);
      dato = vectorNumeros[i]; //NOTA: ¿por qué no se puede poner directamente el vector? 
      MPI_Ssend(&dato, 1, MPI_LONG_LONG_INT, status.MPI_SOURCE, 88, MPI_COMM_WORLD);
      //printf("proceso 0 envía al resto de procesos (proc %d) los números...\n", status.MPI_SOURCE);
    }
    
    //proc 0 envía tarea envenenada
    for(i=0; i < numProcs-1; i++){	//numProcs-1 porque la tarea envenenada no se envía al proc 0
      MPI_Recv(&token, 1, MPI_CHAR, MPI_ANY_SOURCE, 88, MPI_COMM_WORLD, &status);
      MPI_Ssend(&dato, 1, MPI_LONG_LONG_INT, status.MPI_SOURCE, 99, MPI_COMM_WORLD);
    }
    
    
  }else{ //procesos que envían petición y reciben el número
    
   MPI_Ssend(&token, 1, MPI_CHAR, 0, 88, MPI_COMM_WORLD);
   MPI_Recv(&dato, 1, MPI_LONG_LONG_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD , &status) ; //ANY_TAG porque podría ser un vector 
										      //vacío y, por tanto, tendría el tag 99
