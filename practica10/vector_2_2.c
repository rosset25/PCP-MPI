...
//	printf("i = %d\n", i);
	...
	i += dimVectorLocal;
//	printf("i después de cada iter: %d\n", i);
    ...
    MPI_Recv(&vectorLocal[0], dimVectorLocal, MPI_DOUBLE, 0, 88, MPI_COMM_WORLD, &s ); 
...
/**/
  ...
  if (miId == 0) {
    ...
    sumaFinal=sumaLocal; //Para añadir la suma del procs 0
    
    for(i =1; i < numProcs; i++){
	MPI_Recv(& sumaLocal, 1, MPI_DOUBLE, MPI_ANY_SOURCE, 88, MPI_COMM_WORLD, &s );
	sumaFinal+= sumaLocal;
    }
