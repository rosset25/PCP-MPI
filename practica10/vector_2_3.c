...
/**/
  ...
  if (miId == 0){
    vectorSuma = ( double * ) malloc( numProcs * sizeof( double ) );
    
  }
  
  //recogida de la sumaLocal de todos los procesos en el vector vectorSuma, que lo contiene el prc 0
  MPI_Gather(&sumaLocal, 1, MPI_DOUBLE, vectorSuma, numProcs, MPI_DOUBLE, 0 , MPI_COMM_WORLD); 
